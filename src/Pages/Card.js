...
const Card = ({ title, logo, fee, description, rating }) => {
  return (
    <div className="bg-white shadow-md rounded-lg p-6 flex flex-col items-center text-center w-[300px]">
      ...
      <p className="text-3xl font-bold text-[#6FC7AB]">{fee}</p>
      <p className="text-xl font-bold text-[#393939] mb-4">Processing Fee</p>
      ...
      <p className="font-bold text-[#3D3D3D]">
        Rating: <span className="text-[#6FC7AB] text-3xl ">{rating}</span>
      </p>
    </div>
  );
};
